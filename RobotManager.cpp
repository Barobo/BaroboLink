...
void vbox_insert_line(GtkVBox *vbox, const char *str)
{
...
void CRobotManager::generateInteractivePythonProgram(GtkVBox *vbox, bool looped, bool holdOnExit)
{
  string buf;
  string *program = new string();
  char tbuf[200];
  int i, j;
  char tmp[80];
  vbox_insert_line(vbox, "#!/usr/bin/env python");
  vbox_insert_line(vbox, "# Program generated by BaroboLink");
  vbox_insert_line(vbox, "from barobo.linkbot import *");
  vbox_insert_line(vbox, "import time");
  ...
  for(i = 0; i < numConnected(); i++) {
    ...
    buf.assign(tbuf);
    //*program += buf;
    vbox_insert_line(vbox, buf.c_str());
  }
  ...
  if(looped) {
    vbox_insert_line(vbox, "# Set the \"num\" variable to the number of times to loop");
    vbox_insert_line(vbox, "num = 3");
    vbox_insert_line(vbox, "for i in range (0, num):");
  }
  /* Make sure there were connected mobots */
  if(getMobot(0) != NULL) {
    /* Set all of the robot names so they are in order */
