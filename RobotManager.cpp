...
void CRobotManager::generateInteractiveChProgram(GtkBox *vbox, bool looped, bool holdOnExit)
{
  string buf;
  string *program = new string();
  char tbuf[200];
  ...
  char tmp[80];
  box_insert_line(vbox, "/* Program generated by BaroboLink */");
  box_insert_line(vbox, "#include <linkbot.h>");
  ...
  for(i = 0; i < numConnected(); i++) {
    if(getMobot(i)->mobot.formFactor == MOBOTFORM_I) {
      sprintf(tbuf, "CLinkbotI robot%d;", i+1);
    } else if (getMobot(i)->mobot.formFactor == MOBOTFORM_L) {
      sprintf(tbuf, "CLinkbotL robot%d;", i+1);
    } else {
      sprintf(tbuf, "CMobot robot%d;", i+1);
    }
    buf.assign(tbuf);
    //*program += buf;
    box_insert_line(vbox, buf.c_str());
  ...
  if(looped) {
    box_insert_line(vbox, "// Set the \"num\" variable to the number of times to loop");
    box_insert_line(vbox, "int num = 3;");
    box_insert_line(vbox, "int i;");
    box_insert_line(vbox, "for(i = 0; i < num; i++) { ");
  }
  /* Make sure there were connected mobots */
  if(getMobot(0) != NULL) {
    /* Set all of the robot names so they are in order */
    ...
    for(i = 0; i < getMobot(0)->numMotions; i++) {
      ...
      for(j = 0; j < numConnected(); j++) {
        if(numConnected() == 1) {
          //RecordMobot_getPythonMotionStringB(getMobot(j), i, tbuf);
          RecordMobot_getChInteractiveMotionStringB(GTK_WIDGET(vbox), getMobot(j), i);
        } else {
          //RecordMobot_getPythonMotionString(getMobot(j), i, tbuf);
          RecordMobot_getChInteractiveMotionString(GTK_WIDGET(vbox), getMobot(j), i);
        }
        buf.assign(tbuf);
        buf += "\n";
        if(looped) *program += "    ";
...
void CRobotManager::generateInteractiveCppProgram(GtkBox *vbox, bool looped, bool holdOnExit)
{
  string buf;
  string *program = new string();
  char tbuf[200];
